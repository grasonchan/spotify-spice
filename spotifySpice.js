...
`;
    ...
    function handleTracksNamePreview() {
      ...
      trackCondition = (element) =>
        !element.contextTrack.metadata.hidden &&
        element.provider !== 'ad';

      const prevTrack = prevTracks
        .slice()
        .reverse()
        .find(trackCondition);
      ...
      const currentTrackTitle =
        currentTrack.contextTrack.metadata.title;
    ...
    function handleFADBackdrop(event) {
      ...
      const fullAppDisplay = document.querySelector(
        '#full-app-display'
      );
    ...
    function handleIcons() {
      ...
      if (!isFadArtistSvg) {
        ...
        const fadArtistTitle =
          document.querySelector('#fad-artist span');
...
`;
    ...
    Spicetify.PopupModal.addEventListener(
      'click',
